
# BKPS dependency list
## Dependency License Report
_2024-10-23 18:57:28 CEST_
## Apache 2

**1** **Group:** `com.opencsv` **Name:** `opencsv` **Version:** `5.9` 
> - **POM Project URL**: [http://opencsv.sf.net](http://opencsv.sf.net)
> - **POM License**: Apache 2 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)

## Apache 2.0

**2** **Group:** `org.springframework.retry` **Name:** `spring-retry` **Version:** `2.0.7` 
> - **POM Project URL**: [https://www.springsource.org](https://www.springsource.org)
> - **POM License**: Apache 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [spring-retry-2.0.7.jar/META-INF/NOTICE.txt](spring-retry-2.0.7.jar/META-INF/NOTICE.txt)

## Apache License 2.0

**3** **Group:** `io.swagger.core.v3` **Name:** `swagger-annotations` **Version:** `2.2.22` 
> - **Manifest Project URL**: [https://github.com/swagger-api/swagger-core/modules/swagger-annotations](https://github.com/swagger-api/swagger-core/modules/swagger-annotations)
> - **Manifest License**: "Apache License 2.0";link="http://www.apache.org/licenses/LICENSE-2.0.html" (Not Packaged)
> - **POM License**: Apache License 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.html](http://www.apache.org/licenses/LICENSE-2.0.html)
> - **Embedded license files**: [swagger-annotations-2.2.22.jar/META-INF/LICENSE](swagger-annotations-2.2.22.jar/META-INF/LICENSE) 
    - [swagger-annotations-2.2.22.jar/META-INF/NOTICE](swagger-annotations-2.2.22.jar/META-INF/NOTICE)

**4** **Group:** `jakarta.validation` **Name:** `jakarta.validation-api` **Version:** `3.0.2` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://beanvalidation.org](https://beanvalidation.org)
> - **POM License**: Apache License 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)

**5** **Group:** `org.hibernate.validator` **Name:** `hibernate-validator` **Version:** `8.0.1.Final` 
> - **POM License**: Apache License 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)

**6** **Group:** `org.jboss.logging` **Name:** `jboss-logging` **Version:** `3.5.3.Final` 
> - **Project URL**: [http://www.jboss.org](http://www.jboss.org)
> - **POM License**: Apache License 2.0 - [https://repository.jboss.org/licenses/apache-2.0.txt](https://repository.jboss.org/licenses/apache-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)

**7** **Group:** `org.jboss.threads` **Name:** `jboss-threads` **Version:** `3.5.0.Final` 
> - **POM License**: Apache License 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)
> - **Embedded license files**: [jboss-threads-3.5.0.Final.jar/META-INF/LICENSE.txt](jboss-threads-3.5.0.Final.jar/META-INF/LICENSE.txt)

**8** **Group:** `org.jboss.xnio` **Name:** `xnio-api` **Version:** `3.8.8.Final` 
> - **Manifest Project URL**: [http://www.jboss.org](http://www.jboss.org)
> - **POM Project URL**: [http://www.jboss.org/xnio](http://www.jboss.org/xnio)
> - **POM License**: Apache License 2.0 - [http://repository.jboss.org/licenses/apache-2.0.txt](http://repository.jboss.org/licenses/apache-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)

**9** **Group:** `org.jboss.xnio` **Name:** `xnio-nio` **Version:** `3.8.8.Final` 
> - **Manifest Project URL**: [http://www.jboss.org](http://www.jboss.org)
> - **POM License**: Apache License 2.0 - [http://repository.jboss.org/licenses/apache-2.0.txt](http://repository.jboss.org/licenses/apache-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)

**10** **Group:** `org.wildfly.client` **Name:** `wildfly-client-config` **Version:** `1.0.1.Final` 
> - **Manifest Project URL**: [http://www.jboss.org](http://www.jboss.org)
> - **POM License**: Apache License 2.0 - [http://repository.jboss.org/licenses/apache-2.0.txt](http://repository.jboss.org/licenses/apache-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)

**11** **Group:** `org.wildfly.common` **Name:** `wildfly-common` **Version:** `1.5.4.Final` 
> - **POM License**: Apache License 2.0 - [http://repository.jboss.org/licenses/apache-2.0.txt](http://repository.jboss.org/licenses/apache-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)

## Apache License Version 2.0

**12** **Group:** `io.undertow` **Name:** `undertow-core` **Version:** `2.3.13.Final` 
> - **Manifest Project URL**: [http://www.jboss.org](http://www.jboss.org)
> - **POM License**: Apache License Version 2.0 - [http://repository.jboss.org/licenses/apache-2.0.txt](http://repository.jboss.org/licenses/apache-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)

**13** **Group:** `io.undertow` **Name:** `undertow-servlet` **Version:** `2.3.13.Final` 
> - **Manifest Project URL**: [http://www.jboss.org](http://www.jboss.org)
> - **POM License**: Apache License Version 2.0 - [http://repository.jboss.org/licenses/apache-2.0.txt](http://repository.jboss.org/licenses/apache-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)

**14** **Group:** `io.undertow` **Name:** `undertow-websockets-jsr` **Version:** `2.3.13.Final` 
> - **Manifest Project URL**: [http://www.jboss.org](http://www.jboss.org)
> - **POM License**: Apache License Version 2.0 - [http://repository.jboss.org/licenses/apache-2.0.txt](http://repository.jboss.org/licenses/apache-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)

## Apache License, Version 2.0

**15** **Group:** `com.fasterxml` **Name:** `classmate` **Version:** `1.7.0` 
> - **Project URL**: [https://github.com/FasterXML/java-classmate](https://github.com/FasterXML/java-classmate)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: The Apache Software License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [classmate-1.7.0.jar/META-INF/LICENSE](classmate-1.7.0.jar/META-INF/LICENSE) 
    - [classmate-1.7.0.jar/META-INF/NOTICE](classmate-1.7.0.jar/META-INF/NOTICE)

**16** **Group:** `com.fasterxml.jackson.core` **Name:** `jackson-core` **Version:** `2.17.2` 
> - **Project URL**: [https://github.com/FasterXML/jackson-core](https://github.com/FasterXML/jackson-core)
> - **POM License**: Apache License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: The Apache Software License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [jackson-core-2.17.2.jar/META-INF/LICENSE](jackson-core-2.17.2.jar/META-INF/LICENSE) 
    - [jackson-core-2.17.2.jar/META-INF/NOTICE](jackson-core-2.17.2.jar/META-INF/NOTICE)

**17** **Group:** `com.fasterxml.jackson.core` **Name:** `jackson-databind` **Version:** `2.17.2` 
> - **Project URL**: [https://github.com/FasterXML/jackson](https://github.com/FasterXML/jackson)
> - **POM License**: Apache License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: The Apache Software License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [jackson-databind-2.17.2.jar/META-INF/LICENSE](jackson-databind-2.17.2.jar/META-INF/LICENSE) 
    - [jackson-databind-2.17.2.jar/META-INF/NOTICE](jackson-databind-2.17.2.jar/META-INF/NOTICE)

**18** **Group:** `com.fasterxml.jackson.datatype` **Name:** `jackson-datatype-jdk8` **Version:** `2.17.2` 
> - **Manifest Project URL**: [https://github.com/FasterXML/jackson-modules-java8/jackson-datatype-jdk8](https://github.com/FasterXML/jackson-modules-java8/jackson-datatype-jdk8)
> - **POM License**: Apache License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: The Apache Software License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [jackson-datatype-jdk8-2.17.2.jar/META-INF/LICENSE](jackson-datatype-jdk8-2.17.2.jar/META-INF/LICENSE) 
    - [jackson-datatype-jdk8-2.17.2.jar/META-INF/NOTICE](jackson-datatype-jdk8-2.17.2.jar/META-INF/NOTICE)

**19** **Group:** `com.fasterxml.jackson.datatype` **Name:** `jackson-datatype-jsr310` **Version:** `2.17.2` 
> - **Manifest Project URL**: [https://github.com/FasterXML/jackson-modules-java8/jackson-datatype-jsr310](https://github.com/FasterXML/jackson-modules-java8/jackson-datatype-jsr310)
> - **POM License**: Apache License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: The Apache Software License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [jackson-datatype-jsr310-2.17.2.jar/META-INF/LICENSE](jackson-datatype-jsr310-2.17.2.jar/META-INF/LICENSE) 
    - [jackson-datatype-jsr310-2.17.2.jar/META-INF/NOTICE](jackson-datatype-jsr310-2.17.2.jar/META-INF/NOTICE)

**20** **Group:** `com.fasterxml.jackson.module` **Name:** `jackson-module-parameter-names` **Version:** `2.17.2` 
> - **Manifest Project URL**: [https://github.com/FasterXML/jackson-modules-java8/jackson-module-parameter-names](https://github.com/FasterXML/jackson-modules-java8/jackson-module-parameter-names)
> - **POM License**: Apache License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: The Apache Software License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [jackson-module-parameter-names-2.17.2.jar/META-INF/LICENSE](jackson-module-parameter-names-2.17.2.jar/META-INF/LICENSE) 
    - [jackson-module-parameter-names-2.17.2.jar/META-INF/NOTICE](jackson-module-parameter-names-2.17.2.jar/META-INF/NOTICE)

**21** **Group:** `dev.failsafe` **Name:** `failsafe` **Version:** `3.3.2` 
> - **POM License**: Apache License, Version 2.0 - [http://apache.org/licenses/LICENSE-2.0](http://apache.org/licenses/LICENSE-2.0)

**22** **Group:** `io.smallrye` **Name:** `jandex` **Version:** `3.1.2` 
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)

**23** **Group:** `net.bytebuddy` **Name:** `byte-buddy` **Version:** `1.14.18` 
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [byte-buddy-1.14.18.jar/META-INF/LICENSE](byte-buddy-1.14.18.jar/META-INF/LICENSE) 
    - [byte-buddy-1.14.18.jar/META-INF/NOTICE](byte-buddy-1.14.18.jar/META-INF/NOTICE)

**24** **Group:** `net.logstash.logback` **Name:** `logstash-logback-encoder` **Version:** `8.0` 
> - **POM Project URL**: [https://github.com/logfellow/logstash-logback-encoder](https://github.com/logfellow/logstash-logback-encoder)
> - **POM License**: Apache License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0](http://www.apache.org/licenses/LICENSE-2.0)
> - **POM License**: MIT License - [http://www.slf4j.org/license.html](http://www.slf4j.org/license.html)

**25** **Group:** `org.apache.commons` **Name:** `commons-collections4` **Version:** `4.4` 
> - **Project URL**: [https://commons.apache.org/proper/commons-collections/](https://commons.apache.org/proper/commons-collections/)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [commons-collections4-4.4.jar/META-INF/LICENSE.txt](commons-collections4-4.4.jar/META-INF/LICENSE.txt) 
    - [commons-collections4-4.4.jar/META-INF/NOTICE.txt](commons-collections4-4.4.jar/META-INF/NOTICE.txt)

**26** **Group:** `org.apache.httpcomponents.client5` **Name:** `httpclient5` **Version:** `5.3.1` 
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [httpclient5-5.3.1.jar/META-INF/LICENSE](httpclient5-5.3.1.jar/META-INF/LICENSE) 
    - [httpclient5-5.3.1.jar/META-INF/NOTICE](httpclient5-5.3.1.jar/META-INF/NOTICE)

**27** **Group:** `org.apache.httpcomponents.core5` **Name:** `httpcore5` **Version:** `5.2.5` 
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [httpcore5-5.2.5.jar/META-INF/LICENSE](httpcore5-5.2.5.jar/META-INF/LICENSE) 
    - [httpcore5-5.2.5.jar/META-INF/NOTICE](httpcore5-5.2.5.jar/META-INF/NOTICE)

**28** **Group:** `org.apache.httpcomponents.core5` **Name:** `httpcore5-h2` **Version:** `5.2.5` 
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [httpcore5-h2-5.2.5.jar/META-INF/LICENSE](httpcore5-h2-5.2.5.jar/META-INF/LICENSE) 
    - [httpcore5-h2-5.2.5.jar/META-INF/NOTICE](httpcore5-h2-5.2.5.jar/META-INF/NOTICE)

**29** **Group:** `org.apache.tomcat.embed` **Name:** `tomcat-embed-el` **Version:** `10.1.26` 
> - **POM Project URL**: [https://tomcat.apache.org/](https://tomcat.apache.org/)
> - **POM License**: Apache License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [tomcat-embed-el-10.1.26.jar/META-INF/LICENSE](tomcat-embed-el-10.1.26.jar/META-INF/LICENSE) 
    - [tomcat-embed-el-10.1.26.jar/META-INF/NOTICE](tomcat-embed-el-10.1.26.jar/META-INF/NOTICE)

**30** **Group:** `org.liquibase` **Name:** `liquibase-core` **Version:** `4.27.0` 
> - **Manifest Project URL**: [http://www.liquibase.org](http://www.liquibase.org)
> - **POM Project URL**: [http://www.liquibase.com](http://www.liquibase.com)
> - **POM License**: Apache License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0](http://www.apache.org/licenses/LICENSE-2.0)

**31** **Group:** `org.springframework` **Name:** `spring-aop` **Version:** `6.1.11` 
> - **POM Project URL**: [https://github.com/spring-projects/spring-framework](https://github.com/spring-projects/spring-framework)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-aop-6.1.11.jar/META-INF/license.txt](spring-aop-6.1.11.jar/META-INF/license.txt) 
    - [spring-aop-6.1.11.jar/META-INF/notice.txt](spring-aop-6.1.11.jar/META-INF/notice.txt)

**32** **Group:** `org.springframework` **Name:** `spring-aspects` **Version:** `6.1.11` 
> - **POM Project URL**: [https://github.com/spring-projects/spring-framework](https://github.com/spring-projects/spring-framework)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-aspects-6.1.11.jar/META-INF/license.txt](spring-aspects-6.1.11.jar/META-INF/license.txt) 
    - [spring-aspects-6.1.11.jar/META-INF/notice.txt](spring-aspects-6.1.11.jar/META-INF/notice.txt)

**33** **Group:** `org.springframework` **Name:** `spring-beans` **Version:** `6.1.11` 
> - **POM Project URL**: [https://github.com/spring-projects/spring-framework](https://github.com/spring-projects/spring-framework)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-beans-6.1.11.jar/META-INF/license.txt](spring-beans-6.1.11.jar/META-INF/license.txt) 
    - [spring-beans-6.1.11.jar/META-INF/notice.txt](spring-beans-6.1.11.jar/META-INF/notice.txt)

**34** **Group:** `org.springframework` **Name:** `spring-context` **Version:** `6.1.11` 
> - **POM Project URL**: [https://github.com/spring-projects/spring-framework](https://github.com/spring-projects/spring-framework)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-context-6.1.11.jar/META-INF/license.txt](spring-context-6.1.11.jar/META-INF/license.txt) 
    - [spring-context-6.1.11.jar/META-INF/notice.txt](spring-context-6.1.11.jar/META-INF/notice.txt)

**35** **Group:** `org.springframework` **Name:** `spring-core` **Version:** `6.1.11` 
> - **POM Project URL**: [https://github.com/spring-projects/spring-framework](https://github.com/spring-projects/spring-framework)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-core-6.1.11.jar/META-INF/license.txt](spring-core-6.1.11.jar/META-INF/license.txt) 
    - [spring-core-6.1.11.jar/META-INF/notice.txt](spring-core-6.1.11.jar/META-INF/notice.txt)

**36** **Group:** `org.springframework` **Name:** `spring-expression` **Version:** `6.1.11` 
> - **POM Project URL**: [https://github.com/spring-projects/spring-framework](https://github.com/spring-projects/spring-framework)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-expression-6.1.11.jar/META-INF/license.txt](spring-expression-6.1.11.jar/META-INF/license.txt) 
    - [spring-expression-6.1.11.jar/META-INF/notice.txt](spring-expression-6.1.11.jar/META-INF/notice.txt)

**37** **Group:** `org.springframework` **Name:** `spring-jcl` **Version:** `6.1.11` 
> - **POM Project URL**: [https://github.com/spring-projects/spring-framework](https://github.com/spring-projects/spring-framework)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-jcl-6.1.11.jar/META-INF/license.txt](spring-jcl-6.1.11.jar/META-INF/license.txt) 
    - [spring-jcl-6.1.11.jar/META-INF/notice.txt](spring-jcl-6.1.11.jar/META-INF/notice.txt)

**38** **Group:** `org.springframework` **Name:** `spring-jdbc` **Version:** `6.1.11` 
> - **POM Project URL**: [https://github.com/spring-projects/spring-framework](https://github.com/spring-projects/spring-framework)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-jdbc-6.1.11.jar/META-INF/license.txt](spring-jdbc-6.1.11.jar/META-INF/license.txt) 
    - [spring-jdbc-6.1.11.jar/META-INF/notice.txt](spring-jdbc-6.1.11.jar/META-INF/notice.txt)

**39** **Group:** `org.springframework` **Name:** `spring-orm` **Version:** `6.1.11` 
> - **POM Project URL**: [https://github.com/spring-projects/spring-framework](https://github.com/spring-projects/spring-framework)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-orm-6.1.11.jar/META-INF/license.txt](spring-orm-6.1.11.jar/META-INF/license.txt) 
    - [spring-orm-6.1.11.jar/META-INF/notice.txt](spring-orm-6.1.11.jar/META-INF/notice.txt)

**40** **Group:** `org.springframework` **Name:** `spring-tx` **Version:** `6.1.11` 
> - **POM Project URL**: [https://github.com/spring-projects/spring-framework](https://github.com/spring-projects/spring-framework)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-tx-6.1.11.jar/META-INF/license.txt](spring-tx-6.1.11.jar/META-INF/license.txt) 
    - [spring-tx-6.1.11.jar/META-INF/notice.txt](spring-tx-6.1.11.jar/META-INF/notice.txt)

**41** **Group:** `org.springframework` **Name:** `spring-web` **Version:** `6.1.11` 
> - **POM Project URL**: [https://github.com/spring-projects/spring-framework](https://github.com/spring-projects/spring-framework)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-web-6.1.11.jar/META-INF/license.txt](spring-web-6.1.11.jar/META-INF/license.txt) 
    - [spring-web-6.1.11.jar/META-INF/notice.txt](spring-web-6.1.11.jar/META-INF/notice.txt)

**42** **Group:** `org.springframework` **Name:** `spring-webmvc` **Version:** `6.1.11` 
> - **POM Project URL**: [https://github.com/spring-projects/spring-framework](https://github.com/spring-projects/spring-framework)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-webmvc-6.1.11.jar/META-INF/license.txt](spring-webmvc-6.1.11.jar/META-INF/license.txt) 
    - [spring-webmvc-6.1.11.jar/META-INF/notice.txt](spring-webmvc-6.1.11.jar/META-INF/notice.txt)

**43** **Group:** `org.springframework.boot` **Name:** `spring-boot` **Version:** `3.3.2` 
> - **POM Project URL**: [https://spring.io/projects/spring-boot](https://spring.io/projects/spring-boot)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-boot-3.3.2.jar/META-INF/LICENSE.txt](spring-boot-3.3.2.jar/META-INF/LICENSE.txt) 
    - [spring-boot-3.3.2.jar/META-INF/NOTICE.txt](spring-boot-3.3.2.jar/META-INF/NOTICE.txt)

**44** **Group:** `org.springframework.boot` **Name:** `spring-boot-autoconfigure` **Version:** `3.3.2` 
> - **POM Project URL**: [https://spring.io/projects/spring-boot](https://spring.io/projects/spring-boot)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-boot-autoconfigure-3.3.2.jar/META-INF/LICENSE.txt](spring-boot-autoconfigure-3.3.2.jar/META-INF/LICENSE.txt) 
    - [spring-boot-autoconfigure-3.3.2.jar/META-INF/NOTICE.txt](spring-boot-autoconfigure-3.3.2.jar/META-INF/NOTICE.txt)

**45** **Group:** `org.springframework.boot` **Name:** `spring-boot-devtools` **Version:** `3.3.2` 
> - **POM Project URL**: [https://spring.io/projects/spring-boot](https://spring.io/projects/spring-boot)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-boot-devtools-3.3.2.jar/META-INF/LICENSE.txt](spring-boot-devtools-3.3.2.jar/META-INF/LICENSE.txt) 
    - [spring-boot-devtools-3.3.2.jar/META-INF/NOTICE.txt](spring-boot-devtools-3.3.2.jar/META-INF/NOTICE.txt)

**46** **Group:** `org.springframework.boot` **Name:** `spring-boot-loader-tools` **Version:** `3.3.2` 
> - **POM Project URL**: [https://spring.io/projects/spring-boot](https://spring.io/projects/spring-boot)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-boot-loader-tools-3.3.2.jar/META-INF/LICENSE.txt](spring-boot-loader-tools-3.3.2.jar/META-INF/LICENSE.txt) 
    - [spring-boot-loader-tools-3.3.2.jar/META-INF/NOTICE.txt](spring-boot-loader-tools-3.3.2.jar/META-INF/NOTICE.txt)

**47** **Group:** `org.springframework.boot` **Name:** `spring-boot-starter` **Version:** `3.3.2` 
> - **POM Project URL**: [https://spring.io/projects/spring-boot](https://spring.io/projects/spring-boot)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-boot-starter-3.3.2.jar/META-INF/LICENSE.txt](spring-boot-starter-3.3.2.jar/META-INF/LICENSE.txt) 
    - [spring-boot-starter-3.3.2.jar/META-INF/NOTICE.txt](spring-boot-starter-3.3.2.jar/META-INF/NOTICE.txt)

**48** **Group:** `org.springframework.boot` **Name:** `spring-boot-starter-aop` **Version:** `3.3.2` 
> - **POM Project URL**: [https://spring.io/projects/spring-boot](https://spring.io/projects/spring-boot)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-boot-starter-aop-3.3.2.jar/META-INF/LICENSE.txt](spring-boot-starter-aop-3.3.2.jar/META-INF/LICENSE.txt) 
    - [spring-boot-starter-aop-3.3.2.jar/META-INF/NOTICE.txt](spring-boot-starter-aop-3.3.2.jar/META-INF/NOTICE.txt)

**49** **Group:** `org.springframework.boot` **Name:** `spring-boot-starter-data-jpa` **Version:** `3.3.2` 
> - **POM Project URL**: [https://spring.io/projects/spring-boot](https://spring.io/projects/spring-boot)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-boot-starter-data-jpa-3.3.2.jar/META-INF/LICENSE.txt](spring-boot-starter-data-jpa-3.3.2.jar/META-INF/LICENSE.txt) 
    - [spring-boot-starter-data-jpa-3.3.2.jar/META-INF/NOTICE.txt](spring-boot-starter-data-jpa-3.3.2.jar/META-INF/NOTICE.txt)

**50** **Group:** `org.springframework.boot` **Name:** `spring-boot-starter-jdbc` **Version:** `3.3.2` 
> - **POM Project URL**: [https://spring.io/projects/spring-boot](https://spring.io/projects/spring-boot)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-boot-starter-jdbc-3.3.2.jar/META-INF/LICENSE.txt](spring-boot-starter-jdbc-3.3.2.jar/META-INF/LICENSE.txt) 
    - [spring-boot-starter-jdbc-3.3.2.jar/META-INF/NOTICE.txt](spring-boot-starter-jdbc-3.3.2.jar/META-INF/NOTICE.txt)

**51** **Group:** `org.springframework.boot` **Name:** `spring-boot-starter-json` **Version:** `3.3.2` 
> - **POM Project URL**: [https://spring.io/projects/spring-boot](https://spring.io/projects/spring-boot)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-boot-starter-json-3.3.2.jar/META-INF/LICENSE.txt](spring-boot-starter-json-3.3.2.jar/META-INF/LICENSE.txt) 
    - [spring-boot-starter-json-3.3.2.jar/META-INF/NOTICE.txt](spring-boot-starter-json-3.3.2.jar/META-INF/NOTICE.txt)

**52** **Group:** `org.springframework.boot` **Name:** `spring-boot-starter-logging` **Version:** `3.3.2` 
> - **POM Project URL**: [https://spring.io/projects/spring-boot](https://spring.io/projects/spring-boot)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-boot-starter-logging-3.3.2.jar/META-INF/LICENSE.txt](spring-boot-starter-logging-3.3.2.jar/META-INF/LICENSE.txt) 
    - [spring-boot-starter-logging-3.3.2.jar/META-INF/NOTICE.txt](spring-boot-starter-logging-3.3.2.jar/META-INF/NOTICE.txt)

**53** **Group:** `org.springframework.boot` **Name:** `spring-boot-starter-security` **Version:** `3.3.2` 
> - **POM Project URL**: [https://spring.io/projects/spring-boot](https://spring.io/projects/spring-boot)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-boot-starter-security-3.3.2.jar/META-INF/LICENSE.txt](spring-boot-starter-security-3.3.2.jar/META-INF/LICENSE.txt) 
    - [spring-boot-starter-security-3.3.2.jar/META-INF/NOTICE.txt](spring-boot-starter-security-3.3.2.jar/META-INF/NOTICE.txt)

**54** **Group:** `org.springframework.boot` **Name:** `spring-boot-starter-undertow` **Version:** `3.3.2` 
> - **POM Project URL**: [https://spring.io/projects/spring-boot](https://spring.io/projects/spring-boot)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-boot-starter-undertow-3.3.2.jar/META-INF/LICENSE.txt](spring-boot-starter-undertow-3.3.2.jar/META-INF/LICENSE.txt) 
    - [spring-boot-starter-undertow-3.3.2.jar/META-INF/NOTICE.txt](spring-boot-starter-undertow-3.3.2.jar/META-INF/NOTICE.txt)

**55** **Group:** `org.springframework.boot` **Name:** `spring-boot-starter-web` **Version:** `3.3.2` 
> - **POM Project URL**: [https://spring.io/projects/spring-boot](https://spring.io/projects/spring-boot)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-boot-starter-web-3.3.2.jar/META-INF/LICENSE.txt](spring-boot-starter-web-3.3.2.jar/META-INF/LICENSE.txt) 
    - [spring-boot-starter-web-3.3.2.jar/META-INF/NOTICE.txt](spring-boot-starter-web-3.3.2.jar/META-INF/NOTICE.txt)

**56** **Group:** `org.springframework.data` **Name:** `spring-data-commons` **Version:** `3.3.2` 
> - **POM Project URL**: [https://spring.io/projects/spring-data](https://spring.io/projects/spring-data)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-data-commons-3.3.2.jar/license.txt](spring-data-commons-3.3.2.jar/license.txt) 
    - [spring-data-commons-3.3.2.jar/notice.txt](spring-data-commons-3.3.2.jar/notice.txt)

**57** **Group:** `org.springframework.data` **Name:** `spring-data-jpa` **Version:** `3.3.2` 
> - **POM Project URL**: [https://projects.spring.io/spring-data-jpa](https://projects.spring.io/spring-data-jpa)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)
> - **Embedded license files**: [spring-data-jpa-3.3.2.jar/license.txt](spring-data-jpa-3.3.2.jar/license.txt) 
    - [spring-data-jpa-3.3.2.jar/notice.txt](spring-data-jpa-3.3.2.jar/notice.txt) 
    - [spring-data-jpa-3.3.2.jar/readme.txt](spring-data-jpa-3.3.2.jar/readme.txt)

**58** **Group:** `org.springframework.security` **Name:** `spring-security-config` **Version:** `6.3.1` 
> - **POM Project URL**: [https://spring.io/projects/spring-security](https://spring.io/projects/spring-security)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)

**59** **Group:** `org.springframework.security` **Name:** `spring-security-core` **Version:** `6.3.1` 
> - **POM Project URL**: [https://spring.io/projects/spring-security](https://spring.io/projects/spring-security)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)

**60** **Group:** `org.springframework.security` **Name:** `spring-security-crypto` **Version:** `6.3.1` 
> - **POM Project URL**: [https://spring.io/projects/spring-security](https://spring.io/projects/spring-security)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)

**61** **Group:** `org.springframework.security` **Name:** `spring-security-data` **Version:** `6.3.1` 
> - **POM Project URL**: [https://spring.io/projects/spring-security](https://spring.io/projects/spring-security)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)

**62** **Group:** `org.springframework.security` **Name:** `spring-security-web` **Version:** `6.3.1` 
> - **POM Project URL**: [https://spring.io/projects/spring-security](https://spring.io/projects/spring-security)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0)

**63** **Group:** `org.yaml` **Name:** `snakeyaml` **Version:** `2.2` 
> - **POM Project URL**: [https://bitbucket.org/snakeyaml/snakeyaml](https://bitbucket.org/snakeyaml/snakeyaml)
> - **POM License**: Apache License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)

## Apache-2.0

**64** **Group:** `commons-codec` **Name:** `commons-codec` **Version:** `1.16.1` 
> - **Project URL**: [https://commons.apache.org/proper/commons-codec/](https://commons.apache.org/proper/commons-codec/)
> - **POM License**: Apache-2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [commons-codec-1.16.1.jar/META-INF/LICENSE.txt](commons-codec-1.16.1.jar/META-INF/LICENSE.txt) 
    - [commons-codec-1.16.1.jar/META-INF/NOTICE.txt](commons-codec-1.16.1.jar/META-INF/NOTICE.txt)

**65** **Group:** `commons-io` **Name:** `commons-io` **Version:** `2.16.1` 
> - **Project URL**: [https://commons.apache.org/proper/commons-io/](https://commons.apache.org/proper/commons-io/)
> - **POM License**: Apache-2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [commons-io-2.16.1.jar/META-INF/LICENSE.txt](commons-io-2.16.1.jar/META-INF/LICENSE.txt) 
    - [commons-io-2.16.1.jar/META-INF/NOTICE.txt](commons-io-2.16.1.jar/META-INF/NOTICE.txt)

**66** **Group:** `net.java.dev.jna` **Name:** `jna` **Version:** `5.14.0` 
> - **POM Project URL**: [https://github.com/java-native-access/jna](https://github.com/java-native-access/jna)
> - **POM License**: Apache-2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: LGPL-2.1-or-later - [https://www.gnu.org/licenses/old-licenses/lgpl-2.1](https://www.gnu.org/licenses/old-licenses/lgpl-2.1)
> - **Embedded license files**: [jna-5.14.0.jar/META-INF/LICENSE](jna-5.14.0.jar/META-INF/LICENSE)

**67** **Group:** `org.apache.commons` **Name:** `commons-compress` **Version:** `1.25.0` 
> - **Project URL**: [https://commons.apache.org/proper/commons-compress/](https://commons.apache.org/proper/commons-compress/)
> - **POM License**: Apache-2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [commons-compress-1.25.0.jar/META-INF/LICENSE.txt](commons-compress-1.25.0.jar/META-INF/LICENSE.txt) 
    - [commons-compress-1.25.0.jar/META-INF/NOTICE.txt](commons-compress-1.25.0.jar/META-INF/NOTICE.txt)

**68** **Group:** `org.apache.commons` **Name:** `commons-lang3` **Version:** `3.16.0` 
> - **Project URL**: [https://commons.apache.org/proper/commons-lang/](https://commons.apache.org/proper/commons-lang/)
> - **POM License**: Apache-2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [commons-lang3-3.16.0.jar/META-INF/LICENSE.txt](commons-lang3-3.16.0.jar/META-INF/LICENSE.txt) 
    - [commons-lang3-3.16.0.jar/META-INF/NOTICE.txt](commons-lang3-3.16.0.jar/META-INF/NOTICE.txt)

**69** **Group:** `org.apache.commons` **Name:** `commons-text` **Version:** `1.12.0` 
> - **Project URL**: [https://commons.apache.org/proper/commons-text](https://commons.apache.org/proper/commons-text)
> - **POM License**: Apache-2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [commons-text-1.12.0.jar/META-INF/LICENSE.txt](commons-text-1.12.0.jar/META-INF/LICENSE.txt) 
    - [commons-text-1.12.0.jar/META-INF/NOTICE.txt](commons-text-1.12.0.jar/META-INF/NOTICE.txt)

**70** **Group:** `org.apache.logging.log4j` **Name:** `log4j-api` **Version:** `2.23.1` 
> - **Manifest License**: "Apache-2.0";link="https://www.apache.org/licenses/LICENSE-2.0.txt" (Not Packaged)
> - **POM License**: Apache-2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [log4j-api-2.23.1.jar/META-INF/LICENSE](log4j-api-2.23.1.jar/META-INF/LICENSE) 
    - [log4j-api-2.23.1.jar/META-INF/NOTICE](log4j-api-2.23.1.jar/META-INF/NOTICE)

**71** **Group:** `org.apache.logging.log4j` **Name:** `log4j-to-slf4j` **Version:** `2.23.1` 
> - **Manifest License**: "Apache-2.0";link="https://www.apache.org/licenses/LICENSE-2.0.txt" (Not Packaged)
> - **POM License**: Apache-2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [log4j-to-slf4j-2.23.1.jar/META-INF/LICENSE](log4j-to-slf4j-2.23.1.jar/META-INF/LICENSE) 
    - [log4j-to-slf4j-2.23.1.jar/META-INF/NOTICE](log4j-to-slf4j-2.23.1.jar/META-INF/NOTICE)

## BSD-2-Clause

**72** **Group:** `org.postgresql` **Name:** `postgresql` **Version:** `42.7.3` 
> - **Manifest Project URL**: [https://jdbc.postgresql.org/](https://jdbc.postgresql.org/)
> - **Manifest License**: BSD-2-Clause (Not Packaged)
> - **POM Project URL**: [https://jdbc.postgresql.org](https://jdbc.postgresql.org)
> - **POM License**: BSD-2-Clause - [https://jdbc.postgresql.org/about/license.html](https://jdbc.postgresql.org/about/license.html)
> - **Embedded license files**: [postgresql-42.7.3.jar/META-INF/LICENSE](postgresql-42.7.3.jar/META-INF/LICENSE) 
    - [postgresql-42.7.3.jar/META-INF/licenses/com.ongres.scram/client-2.1/LICENSE](postgresql-42.7.3.jar/META-INF/licenses/com.ongres.scram/client-2.1/LICENSE) 
    - [postgresql-42.7.3.jar/META-INF/licenses/com.ongres.scram/common-2.1/LICENSE](postgresql-42.7.3.jar/META-INF/licenses/com.ongres.scram/common-2.1/LICENSE) 
    - [postgresql-42.7.3.jar/META-INF/licenses/com.ongres.stringprep/saslprep-1.1/LICENSE](postgresql-42.7.3.jar/META-INF/licenses/com.ongres.stringprep/saslprep-1.1/LICENSE) 
    - [postgresql-42.7.3.jar/META-INF/licenses/com.ongres.stringprep/stringprep-1.1/LICENSE](postgresql-42.7.3.jar/META-INF/licenses/com.ongres.stringprep/stringprep-1.1/LICENSE)

## BSD-3-Clause

**73** **Group:** `org.antlr` **Name:** `antlr4-runtime` **Version:** `4.13.0` 
> - **Manifest Project URL**: [https://www.antlr.org/](https://www.antlr.org/)
> - **POM License**: BSD-3-Clause - [https://www.antlr.org/license.html](https://www.antlr.org/license.html)

## Bouncy Castle Licence

**74** **Group:** `org.bouncycastle` **Name:** `bcpkix-jdk18on` **Version:** `1.77` 
> - **POM Project URL**: [https://www.bouncycastle.org/java.html](https://www.bouncycastle.org/java.html)
> - **POM License**: Bouncy Castle Licence - [https://www.bouncycastle.org/licence.html](https://www.bouncycastle.org/licence.html)

**75** **Group:** `org.bouncycastle` **Name:** `bcprov-jdk18on` **Version:** `1.77` 
> - **POM Project URL**: [https://www.bouncycastle.org/java.html](https://www.bouncycastle.org/java.html)
> - **POM License**: Bouncy Castle Licence - [https://www.bouncycastle.org/licence.html](https://www.bouncycastle.org/licence.html)

**76** **Group:** `org.bouncycastle` **Name:** `bcutil-jdk18on` **Version:** `1.77` 
> - **POM Project URL**: [https://www.bouncycastle.org/java.html](https://www.bouncycastle.org/java.html)
> - **POM License**: Bouncy Castle Licence - [https://www.bouncycastle.org/licence.html](https://www.bouncycastle.org/licence.html)

## CC0 Universal

**77** **Group:** `com.github.peteroupc` **Name:** `datautilities` **Version:** `1.1.0` 
> - **POM Project URL**: [https://github.com/peteroupc/DataUtilities](https://github.com/peteroupc/DataUtilities)
> - **POM License**: CC0 Universal - [http://www.creativecommons.org/publicdomain/zero/1.0/](http://www.creativecommons.org/publicdomain/zero/1.0/)

## CC0-1.0

**78** **Group:** `com.github.peteroupc` **Name:** `numbers` **Version:** `1.8.2` 
> - **POM Project URL**: [https://github.com/peteroupc/Numbers-Java](https://github.com/peteroupc/Numbers-Java)
> - **POM License**: CC0-1.0 - [http://www.creativecommons.org/publicdomain/zero/1.0/](http://www.creativecommons.org/publicdomain/zero/1.0/)

## CDDL 1.1

**79** **Group:** `javax.xml.bind` **Name:** `jaxb-api` **Version:** `2.3.1` 
> - **Manifest Project URL**: [http://www.oracle.com/](http://www.oracle.com/)
> - **POM License**: CDDL 1.1 - [https://oss.oracle.com/licenses/CDDL+GPL-1.1](https://oss.oracle.com/licenses/CDDL+GPL-1.1)
> - **POM License**: GPL2 w/ CPE - [https://oss.oracle.com/licenses/CDDL+GPL-1.1](https://oss.oracle.com/licenses/CDDL+GPL-1.1)
> - **Embedded license files**: [jaxb-api-2.3.1.jar/META-INF/LICENSE.txt](jaxb-api-2.3.1.jar/META-INF/LICENSE.txt)

## EDL 1.0

**80** **Group:** `jakarta.activation` **Name:** `jakarta.activation-api` **Version:** `2.1.3` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://github.com/jakartaee/jaf-api](https://github.com/jakartaee/jaf-api)
> - **POM License**: EDL 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.activation-api-2.1.3.jar/META-INF/LICENSE.md](jakarta.activation-api-2.1.3.jar/META-INF/LICENSE.md) 
    - [jakarta.activation-api-2.1.3.jar/META-INF/NOTICE.md](jakarta.activation-api-2.1.3.jar/META-INF/NOTICE.md)

**81** **Group:** `org.eclipse.angus` **Name:** `angus-activation` **Version:** `2.0.2` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM License**: EDL 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [angus-activation-2.0.2.jar/META-INF/LICENSE.md](angus-activation-2.0.2.jar/META-INF/LICENSE.md) 
    - [angus-activation-2.0.2.jar/META-INF/NOTICE.md](angus-activation-2.0.2.jar/META-INF/NOTICE.md)

## EPL 1.0

**82** **Group:** `com.h2database` **Name:** `h2` **Version:** `2.2.224` 
> - **POM Project URL**: [https://h2database.com](https://h2database.com)
> - **POM License**: EPL 1.0 - [https://opensource.org/licenses/eclipse-1.0.php](https://opensource.org/licenses/eclipse-1.0.php)
> - **POM License**: MPL 2.0 - [https://www.mozilla.org/en-US/MPL/2.0/](https://www.mozilla.org/en-US/MPL/2.0/)

## EPL 2.0

**83** **Group:** `jakarta.annotation` **Name:** `jakarta.annotation-api` **Version:** `2.1.1` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://projects.eclipse.org/projects/ee4j.ca](https://projects.eclipse.org/projects/ee4j.ca)
> - **POM License**: EPL 2.0 - [http://www.eclipse.org/legal/epl-2.0](http://www.eclipse.org/legal/epl-2.0)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **POM License**: GPL2 w/ CPE - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.annotation-api-2.1.1.jar/META-INF/LICENSE.md](jakarta.annotation-api-2.1.1.jar/META-INF/LICENSE.md) 
    - [jakarta.annotation-api-2.1.1.jar/META-INF/NOTICE.md](jakarta.annotation-api-2.1.1.jar/META-INF/NOTICE.md)

**84** **Group:** `jakarta.servlet` **Name:** `jakarta.servlet-api` **Version:** `6.0.0` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://projects.eclipse.org/projects/ee4j.servlet](https://projects.eclipse.org/projects/ee4j.servlet)
> - **POM License**: EPL 2.0 - [http://www.eclipse.org/legal/epl-2.0](http://www.eclipse.org/legal/epl-2.0)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **POM License**: GPL2 w/ CPE - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)

**85** **Group:** `jakarta.transaction` **Name:** `jakarta.transaction-api` **Version:** `2.0.1` 
> - **Manifest Project URL**: [https://github.com/eclipse-ee4j](https://github.com/eclipse-ee4j)
> - **POM Project URL**: [https://projects.eclipse.org/projects/ee4j.jta](https://projects.eclipse.org/projects/ee4j.jta)
> - **POM License**: EPL 2.0 - [http://www.eclipse.org/legal/epl-2.0](http://www.eclipse.org/legal/epl-2.0)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **POM License**: GPL2 w/ CPE - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.transaction-api-2.0.1.jar/META-INF/LICENSE.md](jakarta.transaction-api-2.0.1.jar/META-INF/LICENSE.md) 
    - [jakarta.transaction-api-2.0.1.jar/META-INF/NOTICE.md](jakarta.transaction-api-2.0.1.jar/META-INF/NOTICE.md)

## Eclipse Distribution License - v 1.0

**86** **Group:** `com.sun.istack` **Name:** `istack-commons-runtime` **Version:** `4.1.2` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM License**: Eclipse Distribution License - v 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)

**87** **Group:** `jakarta.xml.bind` **Name:** `jakarta.xml.bind-api` **Version:** `4.0.2` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM License**: Eclipse Distribution License - v 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.xml.bind-api-4.0.2.jar/META-INF/LICENSE.md](jakarta.xml.bind-api-4.0.2.jar/META-INF/LICENSE.md) 
    - [jakarta.xml.bind-api-4.0.2.jar/META-INF/NOTICE.md](jakarta.xml.bind-api-4.0.2.jar/META-INF/NOTICE.md)

**88** **Group:** `org.glassfish.jaxb` **Name:** `jaxb-core` **Version:** `4.0.5` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://eclipse-ee4j.github.io/jaxb-ri/](https://eclipse-ee4j.github.io/jaxb-ri/)
> - **POM License**: Eclipse Distribution License - v 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jaxb-core-4.0.5.jar/META-INF/LICENSE.md](jaxb-core-4.0.5.jar/META-INF/LICENSE.md) 
    - [jaxb-core-4.0.5.jar/META-INF/NOTICE.md](jaxb-core-4.0.5.jar/META-INF/NOTICE.md)

**89** **Group:** `org.glassfish.jaxb` **Name:** `jaxb-runtime` **Version:** `4.0.5` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://eclipse-ee4j.github.io/jaxb-ri/](https://eclipse-ee4j.github.io/jaxb-ri/)
> - **POM License**: Eclipse Distribution License - v 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jaxb-runtime-4.0.5.jar/META-INF/LICENSE.md](jaxb-runtime-4.0.5.jar/META-INF/LICENSE.md) 
    - [jaxb-runtime-4.0.5.jar/META-INF/NOTICE.md](jaxb-runtime-4.0.5.jar/META-INF/NOTICE.md)

**90** **Group:** `org.glassfish.jaxb` **Name:** `txw2` **Version:** `4.0.5` 
> - **POM Project URL**: [https://eclipse-ee4j.github.io/jaxb-ri/](https://eclipse-ee4j.github.io/jaxb-ri/)
> - **POM License**: Eclipse Distribution License - v 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [txw2-4.0.5.jar/META-INF/LICENSE.md](txw2-4.0.5.jar/META-INF/LICENSE.md) 
    - [txw2-4.0.5.jar/META-INF/NOTICE.md](txw2-4.0.5.jar/META-INF/NOTICE.md)

## Eclipse Distribution License v. 1.0

**91** **Group:** `jakarta.persistence` **Name:** `jakarta.persistence-api` **Version:** `3.1.0` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://github.com/eclipse-ee4j/jpa-api](https://github.com/eclipse-ee4j/jpa-api)
> - **POM License**: Eclipse Distribution License v. 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [http://www.eclipse.org/legal/epl-2.0](http://www.eclipse.org/legal/epl-2.0)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.persistence-api-3.1.0.jar/META-INF/LICENSE.md](jakarta.persistence-api-3.1.0.jar/META-INF/LICENSE.md) 
    - [jakarta.persistence-api-3.1.0.jar/META-INF/NOTICE.md](jakarta.persistence-api-3.1.0.jar/META-INF/NOTICE.md)

## Eclipse Public License - v 1.0

**92** **Group:** `ch.qos.logback` **Name:** `logback-classic` **Version:** `1.5.6` 
> - **Manifest Project URL**: [http://www.qos.ch](http://www.qos.ch)
> - **POM License**: Eclipse Public License - v 1.0 - [http://www.eclipse.org/legal/epl-v10.html](http://www.eclipse.org/legal/epl-v10.html)
> - **POM License**: GNU Lesser General Public License - [http://www.gnu.org/licenses/old-licenses/lgpl-2.1.html](http://www.gnu.org/licenses/old-licenses/lgpl-2.1.html)

**93** **Group:** `ch.qos.logback` **Name:** `logback-core` **Version:** `1.5.6` 
> - **Manifest Project URL**: [http://www.qos.ch](http://www.qos.ch)
> - **POM License**: Eclipse Public License - v 1.0 - [http://www.eclipse.org/legal/epl-v10.html](http://www.eclipse.org/legal/epl-v10.html)
> - **POM License**: GNU Lesser General Public License - [http://www.gnu.org/licenses/old-licenses/lgpl-2.1.html](http://www.gnu.org/licenses/old-licenses/lgpl-2.1.html)

## Eclipse Public License - v 2.0

**94** **Group:** `org.aspectj` **Name:** `aspectjweaver` **Version:** `1.9.22.1` 
> - **POM Project URL**: [https://www.eclipse.org/aspectj/](https://www.eclipse.org/aspectj/)
> - **POM License**: Eclipse Public License - v 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)

## Eclipse Public License v. 2.0

**95** **Group:** `com.sun.istack` **Name:** `istack-commons-runtime` **Version:** `4.1.2` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM License**: Eclipse Distribution License - v 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)

**96** **Group:** `jakarta.activation` **Name:** `jakarta.activation-api` **Version:** `2.1.3` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://github.com/jakartaee/jaf-api](https://github.com/jakartaee/jaf-api)
> - **POM License**: EDL 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.activation-api-2.1.3.jar/META-INF/LICENSE.md](jakarta.activation-api-2.1.3.jar/META-INF/LICENSE.md) 
    - [jakarta.activation-api-2.1.3.jar/META-INF/NOTICE.md](jakarta.activation-api-2.1.3.jar/META-INF/NOTICE.md)

**97** **Group:** `jakarta.annotation` **Name:** `jakarta.annotation-api` **Version:** `2.1.1` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://projects.eclipse.org/projects/ee4j.ca](https://projects.eclipse.org/projects/ee4j.ca)
> - **POM License**: EPL 2.0 - [http://www.eclipse.org/legal/epl-2.0](http://www.eclipse.org/legal/epl-2.0)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **POM License**: GPL2 w/ CPE - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.annotation-api-2.1.1.jar/META-INF/LICENSE.md](jakarta.annotation-api-2.1.1.jar/META-INF/LICENSE.md) 
    - [jakarta.annotation-api-2.1.1.jar/META-INF/NOTICE.md](jakarta.annotation-api-2.1.1.jar/META-INF/NOTICE.md)

**98** **Group:** `jakarta.inject` **Name:** `jakarta.inject-api` **Version:** `2.0.1` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://github.com/eclipse-ee4j/injection-api](https://github.com/eclipse-ee4j/injection-api)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **POM License**: The Apache Software License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [jakarta.inject-api-2.0.1.jar/META-INF/LICENSE.txt](jakarta.inject-api-2.0.1.jar/META-INF/LICENSE.txt) 
    - [jakarta.inject-api-2.0.1.jar/META-INF/NOTICE.md](jakarta.inject-api-2.0.1.jar/META-INF/NOTICE.md)

**99** **Group:** `jakarta.persistence` **Name:** `jakarta.persistence-api` **Version:** `3.1.0` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://github.com/eclipse-ee4j/jpa-api](https://github.com/eclipse-ee4j/jpa-api)
> - **POM License**: Eclipse Distribution License v. 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [http://www.eclipse.org/legal/epl-2.0](http://www.eclipse.org/legal/epl-2.0)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.persistence-api-3.1.0.jar/META-INF/LICENSE.md](jakarta.persistence-api-3.1.0.jar/META-INF/LICENSE.md) 
    - [jakarta.persistence-api-3.1.0.jar/META-INF/NOTICE.md](jakarta.persistence-api-3.1.0.jar/META-INF/NOTICE.md)

**100** **Group:** `jakarta.servlet` **Name:** `jakarta.servlet-api` **Version:** `6.0.0` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://projects.eclipse.org/projects/ee4j.servlet](https://projects.eclipse.org/projects/ee4j.servlet)
> - **POM License**: EPL 2.0 - [http://www.eclipse.org/legal/epl-2.0](http://www.eclipse.org/legal/epl-2.0)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **POM License**: GPL2 w/ CPE - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)

**101** **Group:** `jakarta.transaction` **Name:** `jakarta.transaction-api` **Version:** `2.0.1` 
> - **Manifest Project URL**: [https://github.com/eclipse-ee4j](https://github.com/eclipse-ee4j)
> - **POM Project URL**: [https://projects.eclipse.org/projects/ee4j.jta](https://projects.eclipse.org/projects/ee4j.jta)
> - **POM License**: EPL 2.0 - [http://www.eclipse.org/legal/epl-2.0](http://www.eclipse.org/legal/epl-2.0)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **POM License**: GPL2 w/ CPE - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.transaction-api-2.0.1.jar/META-INF/LICENSE.md](jakarta.transaction-api-2.0.1.jar/META-INF/LICENSE.md) 
    - [jakarta.transaction-api-2.0.1.jar/META-INF/NOTICE.md](jakarta.transaction-api-2.0.1.jar/META-INF/NOTICE.md)

**102** **Group:** `jakarta.validation` **Name:** `jakarta.validation-api` **Version:** `3.0.2` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://beanvalidation.org](https://beanvalidation.org)
> - **POM License**: Apache License 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)

**103** **Group:** `jakarta.websocket` **Name:** `jakarta.websocket-api` **Version:** `2.1.1` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://projects.eclipse.org/projects/ee4j.websocket](https://projects.eclipse.org/projects/ee4j.websocket)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.websocket-api-2.1.1.jar/META-INF/LICENSE.md](jakarta.websocket-api-2.1.1.jar/META-INF/LICENSE.md) 
    - [jakarta.websocket-api-2.1.1.jar/META-INF/NOTICE.md](jakarta.websocket-api-2.1.1.jar/META-INF/NOTICE.md)

**104** **Group:** `jakarta.websocket` **Name:** `jakarta.websocket-client-api` **Version:** `2.1.1` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://projects.eclipse.org/projects/ee4j.websocket](https://projects.eclipse.org/projects/ee4j.websocket)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.websocket-client-api-2.1.1.jar/META-INF/LICENSE.md](jakarta.websocket-client-api-2.1.1.jar/META-INF/LICENSE.md) 
    - [jakarta.websocket-client-api-2.1.1.jar/META-INF/NOTICE.md](jakarta.websocket-client-api-2.1.1.jar/META-INF/NOTICE.md)

**105** **Group:** `jakarta.xml.bind` **Name:** `jakarta.xml.bind-api` **Version:** `4.0.2` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM License**: Eclipse Distribution License - v 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.xml.bind-api-4.0.2.jar/META-INF/LICENSE.md](jakarta.xml.bind-api-4.0.2.jar/META-INF/LICENSE.md) 
    - [jakarta.xml.bind-api-4.0.2.jar/META-INF/NOTICE.md](jakarta.xml.bind-api-4.0.2.jar/META-INF/NOTICE.md)

**106** **Group:** `org.eclipse.angus` **Name:** `angus-activation` **Version:** `2.0.2` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM License**: EDL 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [angus-activation-2.0.2.jar/META-INF/LICENSE.md](angus-activation-2.0.2.jar/META-INF/LICENSE.md) 
    - [angus-activation-2.0.2.jar/META-INF/NOTICE.md](angus-activation-2.0.2.jar/META-INF/NOTICE.md)

**107** **Group:** `org.glassfish.jaxb` **Name:** `jaxb-core` **Version:** `4.0.5` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://eclipse-ee4j.github.io/jaxb-ri/](https://eclipse-ee4j.github.io/jaxb-ri/)
> - **POM License**: Eclipse Distribution License - v 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jaxb-core-4.0.5.jar/META-INF/LICENSE.md](jaxb-core-4.0.5.jar/META-INF/LICENSE.md) 
    - [jaxb-core-4.0.5.jar/META-INF/NOTICE.md](jaxb-core-4.0.5.jar/META-INF/NOTICE.md)

**108** **Group:** `org.glassfish.jaxb` **Name:** `jaxb-runtime` **Version:** `4.0.5` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://eclipse-ee4j.github.io/jaxb-ri/](https://eclipse-ee4j.github.io/jaxb-ri/)
> - **POM License**: Eclipse Distribution License - v 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jaxb-runtime-4.0.5.jar/META-INF/LICENSE.md](jaxb-runtime-4.0.5.jar/META-INF/LICENSE.md) 
    - [jaxb-runtime-4.0.5.jar/META-INF/NOTICE.md](jaxb-runtime-4.0.5.jar/META-INF/NOTICE.md)

**109** **Group:** `org.glassfish.jaxb` **Name:** `txw2` **Version:** `4.0.5` 
> - **POM Project URL**: [https://eclipse-ee4j.github.io/jaxb-ri/](https://eclipse-ee4j.github.io/jaxb-ri/)
> - **POM License**: Eclipse Distribution License - v 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [txw2-4.0.5.jar/META-INF/LICENSE.md](txw2-4.0.5.jar/META-INF/LICENSE.md) 
    - [txw2-4.0.5.jar/META-INF/NOTICE.md](txw2-4.0.5.jar/META-INF/NOTICE.md)

## GNU General Public License, version 2 with the GNU Classpath Exception

**110** **Group:** `com.sun.istack` **Name:** `istack-commons-runtime` **Version:** `4.1.2` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM License**: Eclipse Distribution License - v 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)

**111** **Group:** `jakarta.activation` **Name:** `jakarta.activation-api` **Version:** `2.1.3` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://github.com/jakartaee/jaf-api](https://github.com/jakartaee/jaf-api)
> - **POM License**: EDL 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.activation-api-2.1.3.jar/META-INF/LICENSE.md](jakarta.activation-api-2.1.3.jar/META-INF/LICENSE.md) 
    - [jakarta.activation-api-2.1.3.jar/META-INF/NOTICE.md](jakarta.activation-api-2.1.3.jar/META-INF/NOTICE.md)

**112** **Group:** `jakarta.annotation` **Name:** `jakarta.annotation-api` **Version:** `2.1.1` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://projects.eclipse.org/projects/ee4j.ca](https://projects.eclipse.org/projects/ee4j.ca)
> - **POM License**: EPL 2.0 - [http://www.eclipse.org/legal/epl-2.0](http://www.eclipse.org/legal/epl-2.0)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **POM License**: GPL2 w/ CPE - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.annotation-api-2.1.1.jar/META-INF/LICENSE.md](jakarta.annotation-api-2.1.1.jar/META-INF/LICENSE.md) 
    - [jakarta.annotation-api-2.1.1.jar/META-INF/NOTICE.md](jakarta.annotation-api-2.1.1.jar/META-INF/NOTICE.md)

**113** **Group:** `jakarta.inject` **Name:** `jakarta.inject-api` **Version:** `2.0.1` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://github.com/eclipse-ee4j/injection-api](https://github.com/eclipse-ee4j/injection-api)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **POM License**: The Apache Software License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [jakarta.inject-api-2.0.1.jar/META-INF/LICENSE.txt](jakarta.inject-api-2.0.1.jar/META-INF/LICENSE.txt) 
    - [jakarta.inject-api-2.0.1.jar/META-INF/NOTICE.md](jakarta.inject-api-2.0.1.jar/META-INF/NOTICE.md)

**114** **Group:** `jakarta.persistence` **Name:** `jakarta.persistence-api` **Version:** `3.1.0` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://github.com/eclipse-ee4j/jpa-api](https://github.com/eclipse-ee4j/jpa-api)
> - **POM License**: Eclipse Distribution License v. 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [http://www.eclipse.org/legal/epl-2.0](http://www.eclipse.org/legal/epl-2.0)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.persistence-api-3.1.0.jar/META-INF/LICENSE.md](jakarta.persistence-api-3.1.0.jar/META-INF/LICENSE.md) 
    - [jakarta.persistence-api-3.1.0.jar/META-INF/NOTICE.md](jakarta.persistence-api-3.1.0.jar/META-INF/NOTICE.md)

**115** **Group:** `jakarta.servlet` **Name:** `jakarta.servlet-api` **Version:** `6.0.0` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://projects.eclipse.org/projects/ee4j.servlet](https://projects.eclipse.org/projects/ee4j.servlet)
> - **POM License**: EPL 2.0 - [http://www.eclipse.org/legal/epl-2.0](http://www.eclipse.org/legal/epl-2.0)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **POM License**: GPL2 w/ CPE - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)

**116** **Group:** `jakarta.transaction` **Name:** `jakarta.transaction-api` **Version:** `2.0.1` 
> - **Manifest Project URL**: [https://github.com/eclipse-ee4j](https://github.com/eclipse-ee4j)
> - **POM Project URL**: [https://projects.eclipse.org/projects/ee4j.jta](https://projects.eclipse.org/projects/ee4j.jta)
> - **POM License**: EPL 2.0 - [http://www.eclipse.org/legal/epl-2.0](http://www.eclipse.org/legal/epl-2.0)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **POM License**: GPL2 w/ CPE - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.transaction-api-2.0.1.jar/META-INF/LICENSE.md](jakarta.transaction-api-2.0.1.jar/META-INF/LICENSE.md) 
    - [jakarta.transaction-api-2.0.1.jar/META-INF/NOTICE.md](jakarta.transaction-api-2.0.1.jar/META-INF/NOTICE.md)

**117** **Group:** `jakarta.validation` **Name:** `jakarta.validation-api` **Version:** `3.0.2` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://beanvalidation.org](https://beanvalidation.org)
> - **POM License**: Apache License 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)

**118** **Group:** `jakarta.websocket` **Name:** `jakarta.websocket-api` **Version:** `2.1.1` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://projects.eclipse.org/projects/ee4j.websocket](https://projects.eclipse.org/projects/ee4j.websocket)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.websocket-api-2.1.1.jar/META-INF/LICENSE.md](jakarta.websocket-api-2.1.1.jar/META-INF/LICENSE.md) 
    - [jakarta.websocket-api-2.1.1.jar/META-INF/NOTICE.md](jakarta.websocket-api-2.1.1.jar/META-INF/NOTICE.md)

**119** **Group:** `jakarta.websocket` **Name:** `jakarta.websocket-client-api` **Version:** `2.1.1` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://projects.eclipse.org/projects/ee4j.websocket](https://projects.eclipse.org/projects/ee4j.websocket)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.websocket-client-api-2.1.1.jar/META-INF/LICENSE.md](jakarta.websocket-client-api-2.1.1.jar/META-INF/LICENSE.md) 
    - [jakarta.websocket-client-api-2.1.1.jar/META-INF/NOTICE.md](jakarta.websocket-client-api-2.1.1.jar/META-INF/NOTICE.md)

**120** **Group:** `jakarta.xml.bind` **Name:** `jakarta.xml.bind-api` **Version:** `4.0.2` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM License**: Eclipse Distribution License - v 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.xml.bind-api-4.0.2.jar/META-INF/LICENSE.md](jakarta.xml.bind-api-4.0.2.jar/META-INF/LICENSE.md) 
    - [jakarta.xml.bind-api-4.0.2.jar/META-INF/NOTICE.md](jakarta.xml.bind-api-4.0.2.jar/META-INF/NOTICE.md)

**121** **Group:** `org.eclipse.angus` **Name:** `angus-activation` **Version:** `2.0.2` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM License**: EDL 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [angus-activation-2.0.2.jar/META-INF/LICENSE.md](angus-activation-2.0.2.jar/META-INF/LICENSE.md) 
    - [angus-activation-2.0.2.jar/META-INF/NOTICE.md](angus-activation-2.0.2.jar/META-INF/NOTICE.md)

**122** **Group:** `org.glassfish.jaxb` **Name:** `jaxb-core` **Version:** `4.0.5` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://eclipse-ee4j.github.io/jaxb-ri/](https://eclipse-ee4j.github.io/jaxb-ri/)
> - **POM License**: Eclipse Distribution License - v 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jaxb-core-4.0.5.jar/META-INF/LICENSE.md](jaxb-core-4.0.5.jar/META-INF/LICENSE.md) 
    - [jaxb-core-4.0.5.jar/META-INF/NOTICE.md](jaxb-core-4.0.5.jar/META-INF/NOTICE.md)

**123** **Group:** `org.glassfish.jaxb` **Name:** `jaxb-runtime` **Version:** `4.0.5` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://eclipse-ee4j.github.io/jaxb-ri/](https://eclipse-ee4j.github.io/jaxb-ri/)
> - **POM License**: Eclipse Distribution License - v 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jaxb-runtime-4.0.5.jar/META-INF/LICENSE.md](jaxb-runtime-4.0.5.jar/META-INF/LICENSE.md) 
    - [jaxb-runtime-4.0.5.jar/META-INF/NOTICE.md](jaxb-runtime-4.0.5.jar/META-INF/NOTICE.md)

**124** **Group:** `org.glassfish.jaxb` **Name:** `txw2` **Version:** `4.0.5` 
> - **POM Project URL**: [https://eclipse-ee4j.github.io/jaxb-ri/](https://eclipse-ee4j.github.io/jaxb-ri/)
> - **POM License**: Eclipse Distribution License - v 1.0 - [http://www.eclipse.org/org/documents/edl-v10.php](http://www.eclipse.org/org/documents/edl-v10.php)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [txw2-4.0.5.jar/META-INF/LICENSE.md](txw2-4.0.5.jar/META-INF/LICENSE.md) 
    - [txw2-4.0.5.jar/META-INF/NOTICE.md](txw2-4.0.5.jar/META-INF/NOTICE.md)

## GNU Lesser General Public License

**125** **Group:** `ch.qos.logback` **Name:** `logback-classic` **Version:** `1.5.6` 
> - **Manifest Project URL**: [http://www.qos.ch](http://www.qos.ch)
> - **POM License**: Eclipse Public License - v 1.0 - [http://www.eclipse.org/legal/epl-v10.html](http://www.eclipse.org/legal/epl-v10.html)
> - **POM License**: GNU Lesser General Public License - [http://www.gnu.org/licenses/old-licenses/lgpl-2.1.html](http://www.gnu.org/licenses/old-licenses/lgpl-2.1.html)

**126** **Group:** `ch.qos.logback` **Name:** `logback-core` **Version:** `1.5.6` 
> - **Manifest Project URL**: [http://www.qos.ch](http://www.qos.ch)
> - **POM License**: Eclipse Public License - v 1.0 - [http://www.eclipse.org/legal/epl-v10.html](http://www.eclipse.org/legal/epl-v10.html)
> - **POM License**: GNU Lesser General Public License - [http://www.gnu.org/licenses/old-licenses/lgpl-2.1.html](http://www.gnu.org/licenses/old-licenses/lgpl-2.1.html)

## GNU Library General Public License v2.1 or later

**127** **Group:** `org.hibernate.common` **Name:** `hibernate-commons-annotations` **Version:** `6.0.6.Final` 
> - **POM Project URL**: [http://hibernate.org](http://hibernate.org)
> - **POM License**: GNU Library General Public License v2.1 or later - [http://www.opensource.org/licenses/LGPL-2.1](http://www.opensource.org/licenses/LGPL-2.1)

**128** **Group:** `org.hibernate.orm` **Name:** `hibernate-core` **Version:** `6.5.2.Final` 
> - **Manifest Project URL**: [https://www.hibernate.org/orm/6.5](https://www.hibernate.org/orm/6.5)
> - **POM Project URL**: [https://hibernate.org/orm](https://hibernate.org/orm)
> - **POM License**: GNU Library General Public License v2.1 or later - [https://www.opensource.org/licenses/LGPL-2.1](https://www.opensource.org/licenses/LGPL-2.1)

## GPL2 w/ CPE

**129** **Group:** `jakarta.annotation` **Name:** `jakarta.annotation-api` **Version:** `2.1.1` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://projects.eclipse.org/projects/ee4j.ca](https://projects.eclipse.org/projects/ee4j.ca)
> - **POM License**: EPL 2.0 - [http://www.eclipse.org/legal/epl-2.0](http://www.eclipse.org/legal/epl-2.0)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **POM License**: GPL2 w/ CPE - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.annotation-api-2.1.1.jar/META-INF/LICENSE.md](jakarta.annotation-api-2.1.1.jar/META-INF/LICENSE.md) 
    - [jakarta.annotation-api-2.1.1.jar/META-INF/NOTICE.md](jakarta.annotation-api-2.1.1.jar/META-INF/NOTICE.md)

**130** **Group:** `jakarta.servlet` **Name:** `jakarta.servlet-api` **Version:** `6.0.0` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://projects.eclipse.org/projects/ee4j.servlet](https://projects.eclipse.org/projects/ee4j.servlet)
> - **POM License**: EPL 2.0 - [http://www.eclipse.org/legal/epl-2.0](http://www.eclipse.org/legal/epl-2.0)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **POM License**: GPL2 w/ CPE - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)

**131** **Group:** `jakarta.transaction` **Name:** `jakarta.transaction-api` **Version:** `2.0.1` 
> - **Manifest Project URL**: [https://github.com/eclipse-ee4j](https://github.com/eclipse-ee4j)
> - **POM Project URL**: [https://projects.eclipse.org/projects/ee4j.jta](https://projects.eclipse.org/projects/ee4j.jta)
> - **POM License**: EPL 2.0 - [http://www.eclipse.org/legal/epl-2.0](http://www.eclipse.org/legal/epl-2.0)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **POM License**: GPL2 w/ CPE - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **Embedded license files**: [jakarta.transaction-api-2.0.1.jar/META-INF/LICENSE.md](jakarta.transaction-api-2.0.1.jar/META-INF/LICENSE.md) 
    - [jakarta.transaction-api-2.0.1.jar/META-INF/NOTICE.md](jakarta.transaction-api-2.0.1.jar/META-INF/NOTICE.md)

**132** **Group:** `javax.xml.bind` **Name:** `jaxb-api` **Version:** `2.3.1` 
> - **Manifest Project URL**: [http://www.oracle.com/](http://www.oracle.com/)
> - **POM License**: CDDL 1.1 - [https://oss.oracle.com/licenses/CDDL+GPL-1.1](https://oss.oracle.com/licenses/CDDL+GPL-1.1)
> - **POM License**: GPL2 w/ CPE - [https://oss.oracle.com/licenses/CDDL+GPL-1.1](https://oss.oracle.com/licenses/CDDL+GPL-1.1)
> - **Embedded license files**: [jaxb-api-2.3.1.jar/META-INF/LICENSE.txt](jaxb-api-2.3.1.jar/META-INF/LICENSE.txt)

## LGPL-2.1-or-later

**133** **Group:** `net.java.dev.jna` **Name:** `jna` **Version:** `5.14.0` 
> - **POM Project URL**: [https://github.com/java-native-access/jna](https://github.com/java-native-access/jna)
> - **POM License**: Apache-2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: LGPL-2.1-or-later - [https://www.gnu.org/licenses/old-licenses/lgpl-2.1](https://www.gnu.org/licenses/old-licenses/lgpl-2.1)
> - **Embedded license files**: [jna-5.14.0.jar/META-INF/LICENSE](jna-5.14.0.jar/META-INF/LICENSE)

## MIT License

**134** **Group:** `net.logstash.logback` **Name:** `logstash-logback-encoder` **Version:** `8.0` 
> - **POM Project URL**: [https://github.com/logfellow/logstash-logback-encoder](https://github.com/logfellow/logstash-logback-encoder)
> - **POM License**: Apache License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0](http://www.apache.org/licenses/LICENSE-2.0)
> - **POM License**: MIT License - [http://www.slf4j.org/license.html](http://www.slf4j.org/license.html)

**135** **Group:** `org.slf4j` **Name:** `jul-to-slf4j` **Version:** `2.0.13` 
> - **Project URL**: [http://www.slf4j.org](http://www.slf4j.org)
> - **POM License**: MIT License - [http://www.opensource.org/licenses/mit-license.php](http://www.opensource.org/licenses/mit-license.php)
> - **Embedded license files**: [jul-to-slf4j-2.0.13.jar/META-INF/LICENSE.txt](jul-to-slf4j-2.0.13.jar/META-INF/LICENSE.txt)

**136** **Group:** `org.slf4j` **Name:** `slf4j-api` **Version:** `2.0.13` 
> - **Project URL**: [http://www.slf4j.org](http://www.slf4j.org)
> - **POM License**: MIT License - [http://www.opensource.org/licenses/mit-license.php](http://www.opensource.org/licenses/mit-license.php)
> - **Embedded license files**: [slf4j-api-2.0.13.jar/META-INF/LICENSE.txt](slf4j-api-2.0.13.jar/META-INF/LICENSE.txt)

## MPL 2.0

**137** **Group:** `com.h2database` **Name:** `h2` **Version:** `2.2.224` 
> - **POM Project URL**: [https://h2database.com](https://h2database.com)
> - **POM License**: EPL 1.0 - [https://opensource.org/licenses/eclipse-1.0.php](https://opensource.org/licenses/eclipse-1.0.php)
> - **POM License**: MPL 2.0 - [https://www.mozilla.org/en-US/MPL/2.0/](https://www.mozilla.org/en-US/MPL/2.0/)

## Public Domain

**138** **Group:** `io.undertow` **Name:** `undertow-core` **Version:** `2.3.13.Final` 
> - **Manifest Project URL**: [http://www.jboss.org](http://www.jboss.org)
> - **POM License**: Apache License Version 2.0 - [http://repository.jboss.org/licenses/apache-2.0.txt](http://repository.jboss.org/licenses/apache-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)

**139** **Group:** `io.undertow` **Name:** `undertow-servlet` **Version:** `2.3.13.Final` 
> - **Manifest Project URL**: [http://www.jboss.org](http://www.jboss.org)
> - **POM License**: Apache License Version 2.0 - [http://repository.jboss.org/licenses/apache-2.0.txt](http://repository.jboss.org/licenses/apache-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)

**140** **Group:** `io.undertow` **Name:** `undertow-websockets-jsr` **Version:** `2.3.13.Final` 
> - **Manifest Project URL**: [http://www.jboss.org](http://www.jboss.org)
> - **POM License**: Apache License Version 2.0 - [http://repository.jboss.org/licenses/apache-2.0.txt](http://repository.jboss.org/licenses/apache-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)

**141** **Group:** `org.jboss.logging` **Name:** `jboss-logging` **Version:** `3.5.3.Final` 
> - **Project URL**: [http://www.jboss.org](http://www.jboss.org)
> - **POM License**: Apache License 2.0 - [https://repository.jboss.org/licenses/apache-2.0.txt](https://repository.jboss.org/licenses/apache-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)

**142** **Group:** `org.jboss.threads` **Name:** `jboss-threads` **Version:** `3.5.0.Final` 
> - **POM License**: Apache License 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)
> - **Embedded license files**: [jboss-threads-3.5.0.Final.jar/META-INF/LICENSE.txt](jboss-threads-3.5.0.Final.jar/META-INF/LICENSE.txt)

**143** **Group:** `org.jboss.xnio` **Name:** `xnio-api` **Version:** `3.8.8.Final` 
> - **Manifest Project URL**: [http://www.jboss.org](http://www.jboss.org)
> - **POM Project URL**: [http://www.jboss.org/xnio](http://www.jboss.org/xnio)
> - **POM License**: Apache License 2.0 - [http://repository.jboss.org/licenses/apache-2.0.txt](http://repository.jboss.org/licenses/apache-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)

**144** **Group:** `org.jboss.xnio` **Name:** `xnio-nio` **Version:** `3.8.8.Final` 
> - **Manifest Project URL**: [http://www.jboss.org](http://www.jboss.org)
> - **POM License**: Apache License 2.0 - [http://repository.jboss.org/licenses/apache-2.0.txt](http://repository.jboss.org/licenses/apache-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)

**145** **Group:** `org.wildfly.client` **Name:** `wildfly-client-config` **Version:** `1.0.1.Final` 
> - **Manifest Project URL**: [http://www.jboss.org](http://www.jboss.org)
> - **POM License**: Apache License 2.0 - [http://repository.jboss.org/licenses/apache-2.0.txt](http://repository.jboss.org/licenses/apache-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)

**146** **Group:** `org.wildfly.common` **Name:** `wildfly-common` **Version:** `1.5.4.Final` 
> - **POM License**: Apache License 2.0 - [http://repository.jboss.org/licenses/apache-2.0.txt](http://repository.jboss.org/licenses/apache-2.0.txt)
> - **POM License**: Public Domain - [http://repository.jboss.org/licenses/cc0-1.0.txt](http://repository.jboss.org/licenses/cc0-1.0.txt)

## The Apache Software License, Version 2.0

**147** **Group:** `com.fasterxml` **Name:** `classmate` **Version:** `1.7.0` 
> - **Project URL**: [https://github.com/FasterXML/java-classmate](https://github.com/FasterXML/java-classmate)
> - **POM License**: Apache License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: The Apache Software License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [classmate-1.7.0.jar/META-INF/LICENSE](classmate-1.7.0.jar/META-INF/LICENSE) 
    - [classmate-1.7.0.jar/META-INF/NOTICE](classmate-1.7.0.jar/META-INF/NOTICE)

**148** **Group:** `com.fasterxml.jackson.core` **Name:** `jackson-annotations` **Version:** `2.17.2` 
> - **Project URL**: [https://github.com/FasterXML/jackson](https://github.com/FasterXML/jackson)
> - **POM License**: The Apache Software License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [jackson-annotations-2.17.2.jar/META-INF/LICENSE](jackson-annotations-2.17.2.jar/META-INF/LICENSE) 
    - [jackson-annotations-2.17.2.jar/META-INF/NOTICE](jackson-annotations-2.17.2.jar/META-INF/NOTICE)

**149** **Group:** `com.fasterxml.jackson.core` **Name:** `jackson-core` **Version:** `2.17.2` 
> - **Project URL**: [https://github.com/FasterXML/jackson-core](https://github.com/FasterXML/jackson-core)
> - **POM License**: Apache License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: The Apache Software License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [jackson-core-2.17.2.jar/META-INF/LICENSE](jackson-core-2.17.2.jar/META-INF/LICENSE) 
    - [jackson-core-2.17.2.jar/META-INF/NOTICE](jackson-core-2.17.2.jar/META-INF/NOTICE)

**150** **Group:** `com.fasterxml.jackson.core` **Name:** `jackson-databind` **Version:** `2.17.2` 
> - **Project URL**: [https://github.com/FasterXML/jackson](https://github.com/FasterXML/jackson)
> - **POM License**: Apache License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: The Apache Software License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [jackson-databind-2.17.2.jar/META-INF/LICENSE](jackson-databind-2.17.2.jar/META-INF/LICENSE) 
    - [jackson-databind-2.17.2.jar/META-INF/NOTICE](jackson-databind-2.17.2.jar/META-INF/NOTICE)

**151** **Group:** `com.fasterxml.jackson.datatype` **Name:** `jackson-datatype-jdk8` **Version:** `2.17.2` 
> - **Manifest Project URL**: [https://github.com/FasterXML/jackson-modules-java8/jackson-datatype-jdk8](https://github.com/FasterXML/jackson-modules-java8/jackson-datatype-jdk8)
> - **POM License**: Apache License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: The Apache Software License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [jackson-datatype-jdk8-2.17.2.jar/META-INF/LICENSE](jackson-datatype-jdk8-2.17.2.jar/META-INF/LICENSE) 
    - [jackson-datatype-jdk8-2.17.2.jar/META-INF/NOTICE](jackson-datatype-jdk8-2.17.2.jar/META-INF/NOTICE)

**152** **Group:** `com.fasterxml.jackson.datatype` **Name:** `jackson-datatype-jsr310` **Version:** `2.17.2` 
> - **Manifest Project URL**: [https://github.com/FasterXML/jackson-modules-java8/jackson-datatype-jsr310](https://github.com/FasterXML/jackson-modules-java8/jackson-datatype-jsr310)
> - **POM License**: Apache License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: The Apache Software License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [jackson-datatype-jsr310-2.17.2.jar/META-INF/LICENSE](jackson-datatype-jsr310-2.17.2.jar/META-INF/LICENSE) 
    - [jackson-datatype-jsr310-2.17.2.jar/META-INF/NOTICE](jackson-datatype-jsr310-2.17.2.jar/META-INF/NOTICE)

**153** **Group:** `com.fasterxml.jackson.module` **Name:** `jackson-module-parameter-names` **Version:** `2.17.2` 
> - **Manifest Project URL**: [https://github.com/FasterXML/jackson-modules-java8/jackson-module-parameter-names](https://github.com/FasterXML/jackson-modules-java8/jackson-module-parameter-names)
> - **POM License**: Apache License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **POM License**: The Apache Software License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [jackson-module-parameter-names-2.17.2.jar/META-INF/LICENSE](jackson-module-parameter-names-2.17.2.jar/META-INF/LICENSE) 
    - [jackson-module-parameter-names-2.17.2.jar/META-INF/NOTICE](jackson-module-parameter-names-2.17.2.jar/META-INF/NOTICE)

**154** **Group:** `com.zaxxer` **Name:** `HikariCP` **Version:** `5.1.0` 
> - **Manifest Project URL**: [https://github.com/brettwooldridge](https://github.com/brettwooldridge)
> - **POM Project URL**: [https://github.com/brettwooldridge/HikariCP](https://github.com/brettwooldridge/HikariCP)
> - **POM License**: The Apache Software License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)

**155** **Group:** `io.micrometer` **Name:** `micrometer-commons` **Version:** `1.13.2` 
> - **POM Project URL**: [https://github.com/micrometer-metrics/micrometer](https://github.com/micrometer-metrics/micrometer)
> - **POM License**: The Apache Software License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [micrometer-commons-1.13.2.jar/META-INF/LICENSE](micrometer-commons-1.13.2.jar/META-INF/LICENSE) 
    - [micrometer-commons-1.13.2.jar/META-INF/NOTICE](micrometer-commons-1.13.2.jar/META-INF/NOTICE)

**156** **Group:** `io.micrometer` **Name:** `micrometer-observation` **Version:** `1.13.2` 
> - **POM Project URL**: [https://github.com/micrometer-metrics/micrometer](https://github.com/micrometer-metrics/micrometer)
> - **POM License**: The Apache Software License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [micrometer-observation-1.13.2.jar/META-INF/LICENSE](micrometer-observation-1.13.2.jar/META-INF/LICENSE) 
    - [micrometer-observation-1.13.2.jar/META-INF/NOTICE](micrometer-observation-1.13.2.jar/META-INF/NOTICE)

**157** **Group:** `jakarta.inject` **Name:** `jakarta.inject-api` **Version:** `2.0.1` 
> - **Manifest Project URL**: [https://www.eclipse.org](https://www.eclipse.org)
> - **POM Project URL**: [https://github.com/eclipse-ee4j/injection-api](https://github.com/eclipse-ee4j/injection-api)
> - **POM License**: Eclipse Public License v. 2.0 - [https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt](https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.txt)
> - **POM License**: GNU General Public License, version 2 with the GNU Classpath Exception - [https://www.gnu.org/software/classpath/license.html](https://www.gnu.org/software/classpath/license.html)
> - **POM License**: The Apache Software License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [jakarta.inject-api-2.0.1.jar/META-INF/LICENSE.txt](jakarta.inject-api-2.0.1.jar/META-INF/LICENSE.txt) 
    - [jakarta.inject-api-2.0.1.jar/META-INF/NOTICE.md](jakarta.inject-api-2.0.1.jar/META-INF/NOTICE.md)

**158** **Group:** `net.javacrumbs.shedlock` **Name:** `shedlock-core` **Version:** `5.15.0` 
> - **POM License**: The Apache Software License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)

**159** **Group:** `net.javacrumbs.shedlock` **Name:** `shedlock-provider-jdbc-template` **Version:** `5.15.0` 
> - **POM License**: The Apache Software License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)

**160** **Group:** `net.javacrumbs.shedlock` **Name:** `shedlock-spring` **Version:** `5.15.0` 
> - **POM License**: The Apache Software License, Version 2.0 - [https://www.apache.org/licenses/LICENSE-2.0.txt](https://www.apache.org/licenses/LICENSE-2.0.txt)

**161** **Group:** `org.mapstruct` **Name:** `mapstruct` **Version:** `1.6.0` 
> - **POM Project URL**: [https://mapstruct.org/mapstruct/](https://mapstruct.org/mapstruct/)
> - **POM License**: The Apache Software License, Version 2.0 - [http://www.apache.org/licenses/LICENSE-2.0.txt](http://www.apache.org/licenses/LICENSE-2.0.txt)
> - **Embedded license files**: [mapstruct-1.6.0.jar/META-INF/LICENSE.txt](mapstruct-1.6.0.jar/META-INF/LICENSE.txt)

## The MIT License

**162** **Group:** `org.checkerframework` **Name:** `checker-qual` **Version:** `3.42.0` 
> - **Manifest License**: MIT (Not Packaged)
> - **POM Project URL**: [https://checkerframework.org/](https://checkerframework.org/)
> - **POM License**: The MIT License - [http://opensource.org/licenses/MIT](http://opensource.org/licenses/MIT)
> - **Embedded license files**: [checker-qual-3.42.0.jar/META-INF/LICENSE.txt](checker-qual-3.42.0.jar/META-INF/LICENSE.txt)

## Unknown

**163** **Group:** `com.intel.bkp.command` **Name:** `CommandCore` **Version:** `unspecified` 

**164** **Group:** `com.intel.bkp.core` **Name:** `ServiceCore` **Version:** `unspecified` 

**165** **Group:** `com.intel.bkp.crypto` **Name:** `CryptoCore` **Version:** `unspecified` 

**166** **Group:** `com.intel.bkp.fpgacerts` **Name:** `fpgaCertCore` **Version:** `unspecified` 

**167** **Group:** `com.intel.bkp.protocol` **Name:** `ProtocolCore` **Version:** `unspecified` 

**168** **Group:** `com.intel.bkp.utils` **Name:** `Utils` **Version:** `unspecified` 

**169** **Group:** `org.hibernate` **Name:** `hibernate-core` **Version:** `6.6.0.Final` 

## Unlicense

**170** **Group:** `com.upokecenter` **Name:** `cbor` **Version:** `4.5.5` 
> - **POM Project URL**: [https://github.com/peteroupc/CBOR-Java](https://github.com/peteroupc/CBOR-Java)
> - **POM License**: Unlicense - [https://unlicense.org/](https://unlicense.org/)


